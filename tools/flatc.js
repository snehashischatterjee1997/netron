    ...
    _buildTable(type) {
        ...
        this._builder.indent();
            ...
            if (type === this._root.get('root_type')) {
                ...
                this._builder.indent();
                    this._builder.add('return ' + name + '.decode(reader, reader.root);');
                this._builder.outdent();
                this._builder.add('}');
                ...
                if (this._text) {
                    ...
                    this._builder.indent();
                        this._builder.add('return ' + name + '.decodeText(reader, reader.root);');
                    this._builder.outdent();
                    this._builder.add('}');
                }
            }
            ...
            this._builder.indent();
                this._builder.add('const $ = new ' + name + '();');
                    ...
                    if (field.repeated) {
                        if (field.type instanceof flatc.PrimitiveType) {
                            switch (field.type.name) {
                                ...
                                case 'bool': {
                                    this._builder.add('// TODO [bool]');
                                    this._builder.add('$.' + field.name + '= undefined;');
                                    break;
                                }
                                default: {
                            ...
                            this._builder.add('// TODO [Enum]');
                            this._builder.add('$.' + field.name + '= undefined;');
                        }
                        else if (field.type instanceof flatc.Union) {
                            this._builder.add('// TODO [Union]');
                            this._builder.add('$.' + field.name + '= undefined;');
                        }
                        else if (field.type instanceof flatc.Struct) {
                            this._builder.add('// TODO [Struct]');
                            this._builder.add('$.' + field.name + '= undefined;');
                        }
                        else {
                            const fieldType = '$root.' + field.type.parent.name + '.' + field.type.name;
                            this._builder.add('$.' + field.name + ' = reader.array_(position, ' + field.offset + ', ' + fieldType + '.decode);');
                        }
                    }
                    else {
                        if (field.type instanceof flatc.PrimitiveType) {
                            this._builder.add('$.' + field.name + ' = reader.' + field.type.name + '_(position, ' + field.offset + ', ' + field.defaultValue + ');');
                        }
                        else if (field.type instanceof flatc.Enum) {
                            ...
                            this._builder.add('$.' + field.name + ' = reader.union_(position, ' + field.offset + ', ' + unionType + '.decode);');
                        }
                        else {
                            const fieldType = '$root.' + field.type.parent.name + '.' + field.type.name;
                            this._builder.add('$.' + field.name + ' = reader.object(position, ' + field.offset + ', ' + fieldType + '.decode);');
                        }
                    }
                }
                    ...
                    for (const field of type.fields.values()) {
                        if (field.repeated) {
                            if (field.type instanceof flatc.PrimitiveType) {
                                switch (field.type.name) {
                                    case 'int64': {
                                        this._builder.add('// TODO [int64]');
                                        this._builder.add('$.' + field.name + '= undefined;');
                                        break;
                                    }
                                    case 'uint64': {
                                        this._builder.add('// TODO [uint64]');
                                        this._builder.add('$.' + field.name + '= undefined;');
                                        break;
                                    }
                                    case 'string': {
                                ...
                                const fieldType = '$root.' + field.type.parent.name + '.' + field.type.name;
                                this._builder.add('$.' + field.name + ' = reader.array_(json.' + field.name + ', ' + fieldType + '.decodeText);');
                            }
                        }
                        else {
                            if (field.type instanceof flatc.Enum) {
    ...
    _buildStruct(type) {
        ...
        this._builder.indent();
            ...
            this._builder.indent();
                ...
                for (const field of type.fields.values()) {
                    if (field.repeated) {
                        console.log("TODO flatc.js struct support");
                    }
                    else {
                        console.log("TODO flatc.js struct support");
                    }
                }
                this._builder.add('throw new flatbuffers.Error("\'struct\' decode not supported.");');
                // this._builder.add('return $;');
            this._builder.outdent();
            this._builder.add('}');
            ...
            if (this._text) {
                ...
                this._builder.indent();
                    // this._builder.add('const $ = new ' + name + '();');
                    this._builder.add('throw new flatbuffers.Error("\'struct\' text decode not supported.");');
                    // this._builder.add('return $;');
                this._builder.outdent();
                this._builder.add('}');
            }
